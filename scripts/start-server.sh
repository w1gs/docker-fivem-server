...
LAT_V="$(wget -q -O - ${SRV_ADR} | grep 'panel-block  is-active' | head -n 1 | cut -d '"' -f 4 | cut -c 2- | cut -d '-' -f 1 | cut -d '/' -f 2)"
DL_URL=${SRV_ADR}"$(wget -q -O - ${SRV_ADR} | grep 'panel-block  is-active' | head -n 1 | cut -d '"' -f 4 | cut -c 2-)"
